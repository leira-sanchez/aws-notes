...
| S3 - IA (Infrequently Accessed) | 99.99%| 99.999999999% | For infrequently access data that still needs to be accessed quickly. Cheaper than S3 but there is also a retrieval fee.|
|S3 One Zone - IA| 99.99% | 99.999999999% | Lowest cost option for infrequently accessed data that doesn't require multiple AZ resilience.|
| S3 - Intelligent Tiering | - | - | Designed to optimize costs by automatically moving data to the most cost-effective access tier, without impacting performance or operational overhead.|
|S3 Glacier| - | - | Secure, durable, low-cost storage class for data archiving. Default retrieval time takes hours but can be configured to take minutes at a cost.|
|S3 Glacier Deep Archive| -| - | Lowest-cost storage class where a retrieval time of 12 is acceptable.|
